...
public class HorseHelper
{
	...
	public static void sendHorseUpdateInRange(Entity horse)
	{
		...
		CallableHorses.network.send(PacketDistributor.NEAR.with(() -> new TargetPoint(horse.getX(), horse.getY(), horse.getZ(), 32, horse.level().dimension())), new HorseCapSyncPacket(horse.getId(), storedHorse));
	}
	...
	public static CompoundTag sanitizeEntityNBT(CompoundTag nbt)
	{
		if (nbt == null)
			return null;

		CompoundTag cleaned = nbt.copy();
		cleaned.remove("Passengers");
		return cleaned;
	}
}
