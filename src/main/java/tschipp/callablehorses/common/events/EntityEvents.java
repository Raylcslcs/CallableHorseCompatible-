...
public class EntityEvents
{
	...
	public static void onEntityJoinWorld(EntityJoinLevelEvent event)
	{
		...
		if (!world.isClientSide)
		{
			if(joiningEntity instanceof Player player)
			{
				...
				if (!ownedHorse.isEmpty())
				{
					...
					if (data.wasKilled(ownedHorse))
					{
						...
						{
							LivingEntity deadHorse = owner.createHorseEntity(world);
							if (deadHorse != null)
							{
			...
			else if (joiningEntity instanceof LivingEntity && !(joiningEntity instanceof Player))
			{
...
//		}
	...
	public static void onStartTracking(PlayerEvent.StartTracking event)
	{
		...
		if (!player.level().isClientSide)
		{
			...
			{
				IStoredHorse horse = HorseHelper.getHorseCap(target);
				if (horse != null && horse.isOwned())
	...
	public static void onLivingUpdate(LivingTickEvent event)
	{
		...
		if (e instanceof LivingEntity && !(e instanceof Player) && !e.level().isClientSide)
		{
			if (Configs.SERVER.enableDebug.get() || Configs.SERVER.continuousAntiDupeChecking.get())
			{
				IStoredHorse horse = HorseHelper.getHorseCap(e);
				if (horse != null && Configs.SERVER.enableDebug.get())
...
//						e.setPosition(e.getPosX(), -200, e.getPosZ());
	...
	public static void onLivingDeath(LivingDeathEvent event)
	{
		...
		if (!e.level().isClientSide && e instanceof LivingEntity && !(e instanceof Player))
		{
			IStoredHorse horse = HorseHelper.getHorseCap(e);
			if (horse != null && horse.isOwned())
			{
				...
				if (owner != null)
				{
					...
					{
						HorseManager.saveHorse(e);
						LivingEntity deadHorse = horseOwner.createHorseEntity(owner.level());
						if (deadHorse != null)
						{
							HorseManager.prepDeadHorseForRespawning(deadHorse);
